...
# Creates custom class for dataset with flipped images
...
pretrained_path = "./swinv2_finetuned_model.pth" 
...
checkpoint_path = "swinv2_finetuned_2_model.pth"
...
for epoch in range(EPOCHS):
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        if (i + 1) % 200 == 0:  
            print(f"Epoch [{epoch+1}/{EPOCHS}], Batch [{i+1}/{len(train_loader)}], Loss: {loss.item():.4f}, Accuracy: {np.mean(train_accuracies):.4f}")
    ...
    # Custom early stoping mechanism
...
with open(os.path.join(history_dir, "swinv2_finetuned_2_history.json"), "w") as f:
    json.dump(history, f)
print(history)

torch.save(model.state_dict(), "./swinv2_finetuned_2_base.pth")
