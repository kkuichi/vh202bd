...
model = tf.keras.models.load_model('./exp1/xception_final_model.h5')
with open('./exp1/xception_history.json', 'r') as f:
...
checkpoint_fine = callbacks.ModelCheckpoint('xception_finetuned_model.keras', monitor='val_loss', save_best_only=True)

# Adding history callback
...
model.save('./exp2/xception_finetuned_model.keras')

with open('./xception_finetune_history.json', 'w') as f:
